...
const ResultMessage = () => {
  ...
  return (
    ...
    {messageColor == "gray" ?
       ...
       <span>
           Forecast 상황에 따르면 <span className = "text-green-600">연간 배출량이 {annualEmissionVal.toFixed(1).slice(1)}kgCO2e만큼 약 {annualEmissionReductionVal.toFixed(1).slice(1)}% 줄어듭니다</span>. 
           이는 내연기관차 <span className = "text-green-600">{emissionToVehicleCompVal.toFixed(1).slice(1)}대</span>가 전기차로 전환되는 효과와 같습니다. 
       </span>:
       //When messageColor is red
       <span>
           Forecast 상황에 따르면 <span className = "text-red-400">연간 배출량이 {annualEmissionVal.toFixed(1)}kgCO2e만큼 약 {annualEmissionReductionVal.toFixed(1)}% 증가합니다</span>. 
           이는 화석연료 차량 <span className = "text-red-400">{emissionToVehicleCompVal.toFixed(1)}대</span>가 늘어난 효과와 같습니다.
       </span>
    }
    </div>
  );
