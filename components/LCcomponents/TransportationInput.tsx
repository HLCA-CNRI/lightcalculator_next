  ...
  return (
    <div className=" w-[100%] ">
      <div className="  w-[100%] text-lg visible md:hidden">
        {type == "baseline" ? <div className="ml-4 py-2">Baseline</div> : <div className="ml-4 py-2">Forecast</div>}
        ...
        <div className=" w-[100%]  m-5 p-2 flex justify-between">
          ...
          <div className="flex pr-q">
            <input
              ...
              defaultValue={type == "baseline" ? bCommuting.distance : fCommuting.distance}></input>
            <div>km</div>
          </div>
        </div>
      </div>
      ...
      <div className="rounded-lg p-2  m-5">
        <div className="w-[100%] mt-2 flex">
          <div
            ...
            style={{
              width: `${type == "baseline" ? (bCommuting.car / total) * 100 : (fCommuting.car / total) * 100}%`,
            }}
          />
          <div
            className={`${type == "baseline" ? "bg-[#5b9bd5]" : "bg-[#70ad47]"} h-3`}
            style={{
              width: `${type == "baseline" ? (bCommuting.publicTransit / total) * 100 : (fCommuting.publicTransit / total) * 100}%`,
            }}
          />
          <div
            className={`${type == "baseline" ? "bg-[#bdd7ee]" : "bg-[#c5e0b4]"} rounded-r-lg h-3`}
            style={{
              width: `${type == "baseline" ? (bCommuting.walkOrBike / total) * 100 : (fCommuting.walkOrBike / total) * 100}%`,
            }}
          />
        </div>
