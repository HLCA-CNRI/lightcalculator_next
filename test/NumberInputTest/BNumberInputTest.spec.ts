...
test("Baseline Number Input Test", async ({page}) => {
  // Go to https://lc.cnrikorea.com/
  await page.goto("https://lc.cnrikorea.com");
  // Baseline 임직원 수 input을 100 -> 500, 그리고 체크.
  await page.fill('text=Baseline명 >> input[type="number"]', "500");
  await expect.soft(page.locator('text=Baseline명 >> input[type="number"]')).toHaveValue(`500`);
  // Baseline 차량 종류별
  // 휘발류
  const gasoline = page.locator('text=Baseline휘발유 >> input[type="number"]').first();
  await gasoline.fill("1");
  await expect.soft(gasoline).toHaveValue("1");
  // 경유
  const diesel = page.locator('text=Baseline휘발유 >> input[type="number"]').nth(1);
  await diesel.fill("2");
  await expect.soft(diesel).toHaveValue("2");
  // LPG
  const lpg = page.locator('text=Baseline휘발유 >> input[type="number"]').nth(2);
  await lpg.fill("3");
  await expect.soft(lpg).toHaveValue("3");
  // 수소
  const hydrogen = page.locator('text=Baseline휘발유 >> input[type="number"]').nth(3);
  await hydrogen.fill("3");
  await expect.soft(hydrogen).toHaveValue("3");
  // 전기
  const electric = page.locator('text=Baseline휘발유 >> input[type="number"]').nth(4);
  await electric.fill("3");
  await expect.soft(electric).toHaveValue("3");
  // Baseline 차량 연료 비용
  await page.fill('text=Baseline만원 >> input[type="number"]', "10000");
  await expect.soft(page.locator('text=Baseline만원 >> input[type="number"]')).toHaveValue(`10000`);
  // Baseline 출퇴근 거리 및 방식
  // 평균 출퇴근 거리
  const distance = page.locator('text=Baseline평균 출퇴근 거리km >> input[type="number"]').first();
  await distance.fill("20");
  await expect.soft(distance).toHaveValue("20");
  // 자동차 출퇴근 거리
  const car = page.locator('text=Baseline평균 출퇴근 거리km >> input[type="number"]').nth(1);
  await car.fill("40");
  ...
  // 도보 및 자전거
  const walkOrBike = page.locator('text=Baseline평균 출퇴근 거리km >> input[type="number"]').nth(3);
  await walkOrBike.fill("40");
  await expect.soft(walkOrBike).toHaveValue("40");
  // Baseline 사무실 면적
  await page.fill('text=Baseline m2 >> input[type="number"]', `2000`);
  await expect.soft(page.locator('text=Baseline m2 >> input[type="number"]')).toHaveValue(`2000`);
  // Baseline 왕복 비행 출장
  // 아사아
